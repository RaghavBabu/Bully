...
public class ProcessIPPortXmlParser {
	...
	static Map<Integer,Integer> processIDToHBPortMap = new HashMap<Integer, Integer>();
		...
		try {	
			...
			for (int i = 0; i < nodeList.getLength(); i++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					heartBeatPort = eElement.getAttribute("HeartBeatPort");
					
					processIDToIpMap.put(Integer.parseInt(id), ipAddress);
					processIDToPortMap.put(Integer.parseInt(id), Integer.parseInt( eventPort ));
					processIDToHBPortMap.put(Integer.parseInt(id), Integer.parseInt(heartBeatPort));

					System.out.println("Id : "  + id+ ", IP : "+ipAddress+ ",eventPort : "+eventPort+", hearBeatPort : "+heartBeatPort);
				}
			}
		}
		catch (Exception e) {
